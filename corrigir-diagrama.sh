...
echo "🔧 CORRIGINDO TIPAGEM DO DIAGRAMA"
echo "==================================="

# 1. Primeiro, vamos criar/atualizar o arquivo de tipos
echo "📁 Atualizando types/diagrama/index.ts..."
...
echo "✅ Arquivo de tipos atualizado!"
echo ""

# 2. Agora vamos corrigir o Diagrama.tsx com todas as tipagens
echo "📁 Corrigindo components/diagrama/Diagrama.tsx..."
...
const Diagrama: React.FC = () => {
  // Estados principais com tipos explícitos
  ...
  const handleCardDragStart = (id: string, e: React.MouseEvent): void => {
    e.stopPropagation();
    ...
    const newDraggedCards = new Map<string, { startX: number; startY: number }>();
    selectedCards.forEach((cardId: string) => {
  ...
  const updateConnection = (id: string, updates: Partial<Connection>): void => {
  ...
  const deleteConnection = (id: string): void => {
    setConnections((prev: Connection[]) => prev.filter((conn: Connection) => conn.id !== id));
    setSelectedConnections((prev: Set<string>) => {
      const updated = new Set(prev);
      updated.delete(id);
      return updated;
    });
...
echo "✅ Diagrama.tsx corrigido!"
echo ""

# 3. Verificar se o FloatingToolbar também está com os tipos corretos
echo "📁 Verificando FloatingToolbar.tsx..."

if [ -f "src/components/diagrama/FloatingToolbar.tsx" ]; then
  echo "✅ FloatingToolbar.tsx já existe"
else
  echo "⚠️ FloatingToolbar.tsx não encontrado - você precisa criá-lo"
fi
...
echo "   - Todos os parâmetros de setState agora têm tipos explícitos"
echo "   - Imports atualizados com os tipos corretos"
echo "   - Event handlers tipados corretamente"
echo "   - Map com tipos genéricos especificados"
echo "   - Funções com tipos de retorno explícitos"
echo ""
echo "🚀 Para testar:"
echo "   npm run dev"
echo "   Acesse: http://localhost:3000/diagrama"
echo ""
